...
const SimulationGameplay = () => {
  ...
  useEffect(() => {
    if (enemies.length > 0) {
      ...
      enemies.forEach((enemy, index) => {
        initialSpelledWords[index] = new Array(enemy.words.length).fill(false);
  ...
  const handleTimesUp = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      setTimeout(() => {
        ...
        setLives(studentLife);
        enemyInterval.reset();
        console.log(`Proceeding to next word: Word ${currentWordIndex + 1}`);
        setCurrentWordIndex(currentWordIndex + 1); // Move to next word
        setIsPronunciationLocked(true);
      }, (characterDetails.attackFrame / 12) * 2500);
    }

    if (enemyInterval.time === 0 && lives === 1 && isLastWord && isLastEnemy) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log(`Submitting: Enemy ${currentEnemyIndex + 1}/${enemies.length}`);
    console.log(
    ...
    <main className="adventure-wrapper">
            ...
            {enemies.map((enemy, enemyIndex) => {
              ...
              return (
                <div key={enemyIndex} className="enemy-track-list">
                  {enemyIndex > 0 && <div className="enemy-connector"></div>}

                  <div
                    className={`enemy-track-profile ${
                      defeatedEnemies.includes(enemyIndex) ? "defeated" : ""
                    }`}
                  >
                    <img
                  ...
                  {enemy.words.map((word: any, wordIndex: any) => (
                    <React.Fragment key={wordIndex}>
                      <div className="enemy-connector"></div>
                      <div
                        className={`enemy-word ${
                          spelledWords[enemyIndex]?.[wordIndex] ? "spelled" : ""
                        }`}
                      ></div>
                    </React.Fragment>
                  ))}
      ...
      {gameStarted && (
        <section className="adventure-control">
          ...
          >
            <img
              className={isItemLocked ? "img-locked" : "img-unlocked"}
              src={
                isItemLocked
            ...
            {userItems.length > 0 ? (
              userItems.map((userItem) => (
                <div
                  ...
                  className="item-box"
                  onClick={() =>
                    handleUseItem(userItem.itemID.itemID, userItem.itemID.name)
                  }
                >
                  <img
          ...
          <section className="control-input">
            <form onSubmit={handleSubmit}>
              {simulationDetails.simulationDetails?.simulationType ===
              "Syllable" ? (
                <>
                  <p className="syllable-word">{currentWord}</p>
                  <input
