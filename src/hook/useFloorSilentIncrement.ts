...
const useFloorSilentIncrement = () => {
    ...
    const incrementSilentFloor = async () => {
        setLoading(true)
        setError(null)

        try {
            const response = await fetch(
                `${BASE_URL}/user_details/increment_silent_floor?userDetailsID=${userID}`,
                {
                    method: "PATCH",
                    headers: {
...
export default useFloorSilentIncrement
