...
const useFloorSpellingIncrement = () => {
    ...
    const incrementSpellingFloor = async () => {
        setLoading(true)
        setError(null)

        try {
            const response = await fetch(
                `${BASE_URL}/user_details/increment_spelling_floor?userDetailsID=${userID}`,
                {
                    method: "PATCH",
                    headers: {
...
export default useFloorSpellingIncrement
